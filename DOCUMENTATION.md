...
Every `sessionBack` field belongs only to the currently selected partner. `syncSessionBackContent()` first creates a partner-neutral baseline, then overlays that partner's `sessionBack` configuration. It must never inherit another partner's series date, program name, copy, microsite URL, QR code, or logo. Partner Back Card language is standardized for every partner: the feature title and eyebrow are `[PARTNER_NAME] Amyloidosis Program`; the title is `Learn more about the [MONTH] Spotlight Series`; the primary text ends with `from the [PARTNER_NAME] Amyloidosis Program team`; and the secondary text begins `[MONTH] series:` followed by that partner’s specialists. The series month is refreshed from the selected partner's `sessionsApiUrl`: the earliest session dated today or later establishes the active Spotlight series month. The partner ID in that endpoint is the ownership boundary. The microsite URL is refreshed from the exact matching public employer taxonomy term (`field_spotlight_microsite_url`), and the partner logo must come from the same term's `field_image` relationship—not `field_spotlight_logo_display`, which can be empty. `employerTermName` is used only when the selector label differs from the term name. `sessionBack.seriesDate` and `sessionBack.linkText` remain only as offline/API-failure fallbacks. A configured partner QR asset is retained when present; otherwise the card generates a QR code from that same employer-term microsite URL and shows the existing “Scan for Microsite” label. For export, remote employer images are converted to Base64; if their host does not permit CORS (as with the Houston Methodist file), the public image is fetched through the CORS-enabled image proxy before it is embedded.
...
**API endpoint:** `https://somebodytotalkto.com/api/spotlight/microsite/session/search?q={query}&card_type={source}`

For `facebook-promo`, `facebook-promo-multi`, and `foundation`, the selected
card type is sent as `card_type`. This keeps Facebook and Spotlight/Foundation
registration redirects source-scoped even when no saved promo-card row exists.
...
Expected shape from `GET /api/spotlight/microsite/session/search?q=&card_type=`:
...
302 and one-click record, then add one session-scoped key override with the
