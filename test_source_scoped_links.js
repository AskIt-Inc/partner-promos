...
assert(source.includes('card_type='), 'API requests must include source card type');
...
const fit = new Function(
  'MAX_REGISTRATION_DISPLAY_LENGTH',
  `${source.slice(fitStart, fitEnd)}; return fitRegistrationDisplayText;`,
)(28);
...
assert(fit('https://us06web.zoom.us/meeting/register/EiYDZDaVSqKiNgaDDIMSKg').length <= 28);
